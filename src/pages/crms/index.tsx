...
import { getCrms, deleteCrmById } from 'apiSdk/crms';
import { CrmInterface } from 'interfaces/crm';

type ColumnType = ColumnDef<CrmInterface, unknown>;
...
export function CrmListPage(props: CrmListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CrmInterface>>(
    () => `/crms?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CrmInterface) => {
    if (hasAccess('crm', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/crms/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/crms/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('crm', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/crms/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('crm', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="crm" />
        </Flex>

        {hasAccess('crm', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/crms/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
